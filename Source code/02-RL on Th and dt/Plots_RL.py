...
def plot_merit_indices_histograms(Energy_index_mat1,
                                   ...
                                   bins=15,
                                   save_stats=True):
    # === 1. Ottieni directory dello script attuale ===
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # === 2. Definisci una sottocartella risultati_docking ===
...
def plot_traj_cone(error_log, theta, cone_idx, last_idx, out_of_cone, overshoots):
    ...
    # Cone parameters
    h = 100  # Cone height in km (100 meters)
...
def plot_mpc_indices(N_vect, Energy_index_mat1, Time_index_mat, Constraint_index_mat):
    """
    Plots three MPCC performance indices (Energy, Time, Constraint) over prediction steps.

    Parameters:
    - N_vect: array of prediction horizon values
    ...
    for i, (matrix, label, base_cmap) in enumerate([
        ...
        for row_idx in range(n_rows):
            ...
            plt.plot(N_vect, y_vals, 'o-', color=color, label=f'Theta = {theta:.1f}°')
...
def plot_dt_over_time_segment(time_log, dt_log, Th_log, last_idx):
    """
    ...
    plt.plot(time_log[:last_idx], dt_log[:last_idx], '-', color='tab:blue', label='Timestep (dt)')
    plt.plot(time_log[:last_idx], Th_log[:last_idx], '-', color='tab:orange', label='Prediction Horizon (Th)')
    plt.xlabel('Time [s]'); plt.ylabel('[s]')
    plt.title('Timestep and Prediction Horizon over Time')
