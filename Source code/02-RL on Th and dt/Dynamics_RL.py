...
def geopotential(r_vec):
    ...
    sin_phi = z / r  # seno di latitudine geocentrica
    ...
    # Potenziale geopotenziale perturbato
    U = -mu / r * (-J2 * (Re / r)**2 * P2 -
                    J3 * (Re / r)**3 * P3 -
                    J4 * (Re / r)**4 * P4 -
                    J5 * (Re / r)**5 * P5)
...
def J_acc(r_vec):
    # Calcolo numerico del gradiente del potenziale (approccio semplice)
    eps = 1e-1  # piccolo passo di perturbazione
    acc = np.zeros(3)
    for i in range(3):
        dr = np.zeros(3)
        dr[i] = eps
        ...
        acc[i] = -(U_plus - U_minus) / (2 * eps)  # derivata centrale
    return acc
