...
#              The RL agent tunes MPC parameters (dt, Th) to optimize energy,
...
from Utils_RL import random_chaser_state, sun_pos, kep2car, eci2lvlh, control_pert
...
class DockingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        r_min = self.d0_min
        r_max = self.d0_max
        theta_max= self.theta0_max
        self.chaser = random_chaser_state(self.target, theta_max, r_min, r_max)
        self.d0 = np.linalg.norm(self.target-self.chaser)

        self.y_max=self.d0*1000*np.sin(self.theta0_max*np.pi/180)
        self.delta_x_max= np.array([self.y_max, self.d0*1000, self.y_max, 0.2, 1, 0.2 ])/1000
    ...
    def step(self, action):
        ...
        self.cone_violation = dock_err['r2'] / (((dock_err['y'] + 6e-4) * np.tan(self.theta))**2)
        ...
        #Calculate reward
        reward =  self.compute_intermediate_reward(dock_err, comp_time, np.linalg.norm(u_clipped))
    ...
    def compute_intermediate_reward(self, dock_err, comp_time, u):
        ...
        e_term = w_e * u/self.u_max * self.dt
        c_term = w_c * cone_violation * self.dt
        t_term = w_t * comp_time **2
    ...
    def compute_final_reward(self, done):
        ...
        if self.docked:
            reward = 100
            print(f"Total intermediate energy reward     ≈ -{self.total_inter_r_energy:.2e}")
            print(f"Total intermediate constraint reward  ≈ -{self.total_inter_r_constraint:.2e}")
            print(f"Total intermediate computation reward  ≈ -{self.total_inter_r_computation:.2e}")
            total = self.total_inter_r_energy + self.total_inter_r_constraint + self.total_inter_r_computation 
            print(f"Total intermediate reward             ≈ -{total:.2e}")
        else:
            reward = -500
        return reward
