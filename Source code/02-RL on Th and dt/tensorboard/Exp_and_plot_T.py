...
class TensorBoardExporter:
    ...
    def __init__(self, tensorboard_dir="./tensorboard/"):
        self.tensorboard_dir = r"C:\Users\caval\OneDrive - Politecnico di Milano\Thesis\Code\RL on Th and dt\tensorboard\RecurrentPPO_1"
        self.data = {}
    ...
    def export_all_formats(self, df, base_filename="tensor  board_export", output_dir=r"C:\Users\caval\OneDrive - Politecnico di Milano\Thesis\Code\RL on R\tensorboard"):
        """Esporta i dati in diversi formati in una directory specifica"""
        
        # Path hardcoded dove salvare i file
        output_dir = r"C:\Users\caval\OneDrive - Politecnico di Milano\Thesis\Code\RL on Th and dt\tensorboard"
    ...
    def export_tensorboard_data(self, output_prefix="tensorboard_export", output_dir=r"C:\Users\caval\OneDrive - Politecnico di Milano\Thesis\Code\RL on Th and dt\tensorboard"):
...
# === Load data ===
csv_path =r"C:\Users\caval\OneDrive - Politecnico di Milano\Thesis\Code\RL on Th and dt\tensorboard\tensorboard_export.csv"
...
plt.suptitle("Action Stats, Training Metrics, Rollout & Eval Indices", fontsize=16)

# === 1. Real Actions Th ===
plt.subplot(2, 2, 1)
for label, color in zip(['max', 'mean', 'min'], colors[:3]):
    col = f'episode/real_actions_Th_{label}'
    plt.plot(df['step_k'], df[col], color=color, linewidth=1, alpha=0.6)
    plt.scatter(df['step_k'], df[col], label=f'Th {label.capitalize()}', color=color, s=10)

plt.fill_between(df['step_k'],
                 df['episode/real_actions_Th_mean'] - df['episode/real_actions_Th_std'],
                 df['episode/real_actions_Th_mean'] + df['episode/real_actions_Th_std'],
                 color=colors[1], alpha=0.2, label='Th ±1 Std')
plt.title('Real Actions Th')
...
plt.subplot(2, 2, 2)
for label, color in zip(['max', 'mean', 'min'], colors[:3]):
    col = f'episode/real_actions_dt_{label}'
    plt.plot(df['step_k'], df[col], color=color, linewidth=1, alpha=0.6)
    plt.scatter(df['step_k'], df[col], label=f'dt {label.capitalize()}', color=color, s=10)

plt.fill_between(df['step_k'],
                 df['episode/real_actions_dt_mean'] - df['episode/real_actions_dt_std'],
                 df['episode/real_actions_dt_mean'] + df['episode/real_actions_dt_std'],
                 color=colors[1], alpha=0.2, label='dt ±1 Std')
plt.title('Real Actions dt')
plt.xlabel('Training Steps (k)')
plt.ylabel('Value')
plt.legend()
plt.grid(True)
...
if 'time/fps' in df.columns:
    plt.plot(df['step_k'], df['time/fps'], color=colors[0], linewidth=1.5)
...
plt.suptitle("Figure 2 - Episode Performance", fontsize=14)
...
plt.plot(raw['step_k'], ma, color=colors[0], linewidth=2.0, label='Moving Average')
...
# # === 5. Indici Rollout ===
# plt.subplot(2, 2, 3)
# rollout_indices = [col for col in df.columns if col.startswith('episode_perf/')]
# if rollout_indices:
#     for i, col in enumerate(rollout_indices):
#         plt.plot(df['step_k'], df[col], label=col, linewidth=1.2)
# plt.title('Rollout Indices')
# plt.xlabel('Training Steps (k)')
# plt.ylabel('Value')
