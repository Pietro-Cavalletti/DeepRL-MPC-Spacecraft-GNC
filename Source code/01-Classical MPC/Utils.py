...
def kep2car(a, e, i, Ω, ω, theta, mu):
    """
    Converts Keplerian orbital elements to ECI Cartesian state (position and velocity).
    """
    ...
    return np.concatenate((R_mat @ r_pf, R_mat @ v_pf))

def eci2lvlh(x_target):
    """
    Computes the rotation matrix from ECI to LVLH (Local Vertical, Local Horizontal) frame.
    
    Convention:
      - i: Radial inward (towards Earth center)
...
def rk8_step(state, dt, RHS, mu, r_sun, J2, Drag, control):
    """
    High-order numerical integration step using the Runge-Kutta 8th order 
    (Fehlberg RKF78 coefficients) for precise trajectory propagation.
    """
    # Butcher Tableau coefficients for RKF78
    ...
    # 8th order result combination
    y_next = state + dt * sum(c * ki for c, ki in zip(c_high, k))

    return y_next

# --- STOCHASTIC GENERATION AND PERTURBATIONS ---

def random_chaser_state(state_target, theta_max_deg=9, r_min=100, r_max=300):
    """
    Generates a random initial relative state for the chaser satellite 
    within a specified range and cone angle in the LVLH frame.
    """
    Rot = eci2lvlh(state_target)
    
    # Random distance and spherical coordinates
    ...
    # Random initial relative velocity [km/s]
...
def control_pert(u_ctrl_nom, std_theta_deg=3.0):
    """
    Applies intensity and pointing errors to the nominal control vector 
    to simulate real thruster noise and misalignment.
    """
    ...
    for i in range(3):
        ...
        axis = np.zeros(3)
        axis[(i+1)%3] = 1.0  # Orthogonal axis for rotation
        ...
        R_rot_mat = np.eye(3) + np.sin(theta)*K + (1 - np.cos(theta))*(K @ K)

        ei = np.zeros(3)
        ei[i] = 1.0
        u_pert_i = R_rot_mat @ (ei * ui_pert)
        u_dist += u_pert_i

    return u_dist

# --- EPHEMERIS AND MATH UTILS ---

def sun_pos(day_of_year):
    """
    Computes approximate Sun position in ECI coordinates for a given day of the year.
    """
    # Orbital angle (approximate)
    ...
    return np.array([x, y, z]) * 1.495978707e8  # Convert to km (1 AU)
