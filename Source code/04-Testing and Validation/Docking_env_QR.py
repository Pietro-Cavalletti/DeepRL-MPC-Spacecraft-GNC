...
# Year: 2025
# Description: Gymnasium environment for spacecraft docking using Reinforcement 
#              Learning (PPO) and MPC tuning.
# =============================================================================
...
class DockingEnv(gym.Env):
    ...
    def step(self, action, dt , Th):
        ...
        if self.mpc_failure_count >= 3:
            ...
            obs = np.concatenate([self.err_lvlh/self.delta_x_max, np.zeros(3, dtype=np.float32)]) # Fallback obs
    ...
    def compute_intermediate_reward(self, dock_err, comp_time, u, pos_err, vel_err):
        # 1. Reward Weights (main tuning point)
        w_pos1, w_pos2, w_angle = 100, 1.5, 3
        w_control = 2
        
        # --- GRADUAL SCALING OF ANGULAR PENALTY ---
    ...
    def compute_final_reward(self, done):
        ...
        # Dominant final Reward/Penalties for clear objective
        zeta = 100   # Bonus for successful docking
        kappa = 100  # Penalty for failure
